...
type PanelContent = Tables['panel_content']['Row']
...
// Hook for fetching panel content
export function usePanelContent(tabName: string) {
  const supabase = createSupabaseClient()
  
  return useSupabaseQuery<PanelContent[]>(
    async () => {
      if (!supabase) throw new Error('Supabase not configured')
      return await supabase.from('panel_content').select('*').eq('tab_name', tabName)
    },
    [tabName]
  )
}
