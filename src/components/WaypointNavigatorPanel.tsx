...
import { ChevronLeft, ChevronRight, Camera, MapPin, Mountain, Ruler, Clock, Image, Video, FileText, Info, Lock, Phone, X } from 'lucide-react';
...
interface WaypointNavigatorPanelProps {
  ...
  isOpen: boolean; // Add isOpen prop to control the dialog
  onClose: () => void; // Add onClose prop to close the dialog
}

const WaypointNavigatorPanel: React.FC<WaypointNavigatorPanelProps> = ({ trek, isOpen, onClose }) => {
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="sm:max-w-[425px] p-0">
          <DialogHeader className="p-4 border-b">
            <DialogTitle>No Waypoints</DialogTitle>
            <DialogDescription>
              No waypoints available for this trek.
            </DialogDescription>
          </DialogHeader>
          <div className="p-4 text-center text-muted-foreground">
            <p>Please select a trek with defined waypoints.</p>
          </div>
        </DialogContent>
      </Dialog>
    );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[700px] p-0"> {/* Adjust max-width for the modal size */}
        {/* Header */}
        <DialogHeader className="p-6 border-b flex items-center justify-between flex-row">
          <DialogTitle className="text-2xl font-bold capitalize flex-1">
        ...
        {/* Main Content */}
        <div className="flex flex-col h-full w-full bg-background waypoint-panel">
